...
def sample_from_multiple_json(path_json, nr_samples=50000, json_file_size=50000, total_json_files=1):
    ...
    for file_id in range(total_json_files):
...
def apply_character_threshold(path_json, total_json_files=1, threshold=1000):
    ...
    for file_id in range(0, total_json_files, 1):
        ...
        df = df.append(current_df[current_df["str_len"]>threshold])
    print(df)
    df = df.reset_index()
    df = df.sample(50000)
